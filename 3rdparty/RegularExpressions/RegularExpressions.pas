...
type
  ...
  ['{5E3C2BCA-56C8-46DE-959F-338AF5F69C1A}']
    ...
    function GroupNameFromNumber(const I: Integer): String;

    { Returns the group number that corresponds to the specified group name. }
    function GroupNumberFromName(const Name: String): Integer;
    ...
    function IsMatch(const Input: String; const StartAt: Integer): Boolean; overload;
    ...
    function Match(const Input: String): IMatch; overload;
    function Match(const Input: String; const StartAt: Integer): IMatch; overload;
    function Match(const Input: String; const Beginning, Length: Integer): IMatch; overload;
    ...
    { Searches an input string for all occurrences of a regular expression and
      ...
      You can optionally supply a StartAt parameter to start the search at a
      specific character position (1 to start from the first character). }
    function Matches(const Input: String): IMatchCollection; overload;
    function Matches(const Input: String; const StartAt: Integer): IMatchCollection; overload;
    ...
    function Replace(const Input: String;
      const Evaluator: TMatchEvaluator): String; overload;
    function Replace(const Input, Replacement: String;
      const Count: Integer): String; overload;
    function Replace(const Input: String; const Evaluator: TMatchEvaluator;
      const Count: Integer): String; overload;
    function Replace(const Input, Replacement: String;
      const Count, StartAt: Integer): String; overload;
    function Replace(const Input: String; const Evaluator: TMatchEvaluator;
      const Count, StartAt: Integer): String; overload;
    ...
    function Split(const Input: String; const Count: Integer): TStringArray; overload;
    function Split(const Input: String; const Count, StartAt: Integer): TStringArray; overload;
  ...
  ['{CB7547F0-B797-45EB-9EB7-6F7070918DBE}']
    // Property access methods
    function GetItemByName(const GroupName: String): IGroup;
    ...
    property ItemsByName[const GroupName: String]: IGroup read GetItemByName;
  end;
  ...
  ['{6F221769-F7EA-4100-A0F7-EA692AF04CAC}']
    ...
    function Result(const Replacement: String): String;
...
type
  ...
  private
    type
      ...
      private
        ...
        FStudyData: Pointer;
      public
        constructor Create(const AKey: String; const ACompiledPattern,
          AStudyData: Pointer);
    ...
    type
      ...
      public
        ...
        procedure Append(const Value: AnsiString); overload; inline;
        procedure Append(const Value: AnsiString; const Index,
  ...
  private
    ...
    class function LookupCache(const Key: String;
      out CompiledPattern, StudyData: Pointer): Boolean; static;
    class procedure AddToCache(const Key: String;
      const CompiledPattern, StudyData: Pointer); static;
  private
    constructor Create(const APattern: String; const AOptions: TRegexOptions;
    ...
    function Replace: AnsiString; overload;
    function ExpandSubstitutions(const Replacement: AnsiString): AnsiString;
  ...
  public
    ...
    constructor Create(const APattern: String; const AOptions: TRegexOptions); overload;
    ...
    class function Escape(const Str: String): String; static;
    ...
    (*Converts any escaped characters in the input string.
      ...
      -It replaces the representation of unprintable characters with the
       ...
       and \v.*)
    class function Unescape(const Str: String): String; static;

    { See the IRegex interface for an explanation of these methods. }

    class function IsMatch(const Input, Pattern: String): Boolean; overload; static;
    class function IsMatch(const Input, Pattern: String;
      const Options: TRegexOptions): Boolean; overload; static;
    class function Match(const Input, Pattern: String): IMatch; overload; static;
    class function Match(const Input, Pattern: String;
      const Options: TRegexOptions): IMatch; overload; static;
    class function Matches(const Input, Pattern: String): IMatchCollection; overload; static;
    class function Matches(const Input, Pattern: String;
    ...
    class function Replace(const Input, Pattern: String;
      const Evaluator: TMatchEvaluator): String; overload; static;
    class function Replace(const Input, Pattern, Replacement: String;
      const Options: TRegexOptions): String; overload; static;
    class function Replace(const Input, Pattern: String;
      const Evaluator: TMatchEvaluator; const Options: TRegexOptions): String; overload; static;
...
type
  ...
  private
    ...
    FIndex: Integer;
    FLength: Integer;
    constructor Create(const AValue: AnsiString; const AIndex, ALength: Integer);
  {$ENDREGION 'Internal declarations'}
  private
    { ICapture
...
type
  ...
  private
    ...
    constructor Create(const ARegex: TRegex; const AValue: AnsiString;
...
class procedure TRegex.AddToCache(const Key: String; const CompiledPattern,
  StudyData: Pointer);
var
  I: Integer;
...
begin
  ...
  for Index := 0 to GroupCount do
    ...
    begin
      FGroupMap[TargetIndex].Name := IntToStr(TargetIndex);
  ...
  for Index := 0 to GroupCount do
    ...
    begin
      ...
      Inc(P, 2);
      FGroupMap[TargetIndex].Name := String(P);
      FGroupMap[TargetIndex].CaptureIndex := Index;
      Inc(TargetIndex);
    end;
end;
...
constructor TRegex.Create(const APattern: String; const AOptions: TRegexOptions;
  const AUseCache: Boolean);
var
  CultureIndex: Integer;
  CultureKey, Key: String;
begin
  inherited Create;
  FPattern := UTF8Encode(APattern);
  FOptions := AOptions;
  ...
  if (CultureIndex < 0) then
    ...
    CultureKey := Languages.Ext[CultureIndex];

  Key := IntToStr(Byte(FOptions)) + ':' + CultureKey + ':' + APattern;
...
class function TRegex.Escape(const Str: String): String;
var
  I: Integer;
begin
  Result := Str;
...
function TRegex.ExpandSubstitutions(const Replacement: AnsiString): AnsiString;
...
function TRegex.GroupNameFromNumber(const I: Integer): String;
begin
  Assert(Assigned(FGroupMap));
...
function TRegex.GroupNumberFromName(const Name: String): Integer;
begin
  Assert(Assigned(FGroupMap));
...
class function TRegex.IsMatch(const Input, Pattern: String;
...
function TRegex.IsMatch(const Input: String; const StartAt: Integer): Boolean;
begin
  SetSubject(Input);
...
class function TRegex.LookupCache(const Key: String; out CompiledPattern,
...
function TRegex.Match(const Input: String; const Beginning,
...
class function TRegex.Match(const Input, Pattern: String): IMatch;
var
  Regex: IRegex;
begin
  Regex := TRegex.Create(Pattern, [], True);
...
function TRegex.Match(const Input: String; const StartAt: Integer): IMatch;
begin
  SetSubject(Input);
  FStartOffset := StartAt;
  if (Match) then
...
function TRegex.Matches(const Input: String;
...
class function TRegex.Matches(const Input, Pattern: String;
...
class function TRegex.Matches(const Input, Pattern: String): IMatchCollection;
var
  Regex: IRegex;
begin
  Regex := TRegex.Create(Pattern, [], True);
...
class function TRegex.Replace(const Input, Pattern: String;
  const Evaluator: TMatchEvaluator): String;
var
  Regex: IRegex;
begin
  Regex := TRegex.Create(Pattern, [], True);
...
class function TRegex.Replace(const Input, Pattern, Replacement: String;
  const Options: TRegexOptions): String;
var
  Regex: IRegex;
begin
  Regex := TRegex.Create(Pattern, Options, True);
...
function TRegex.Replace(const Input: String; const Evaluator: TMatchEvaluator;
  const Count: Integer): String;
begin
  FEvaluator := Evaluator;
  try
...
class function TRegex.Replace(const Input, Pattern: String;
  const Evaluator: TMatchEvaluator; const Options: TRegexOptions): String;
var
  Regex: IRegex;
begin
  Regex := TRegex.Create(Pattern, Options, True);
...
function TRegex.Replace: AnsiString;
var
...
begin
  ...
  begin
    ...
    Result := UTF8Encode(FEvaluator(Match));
  end;
...
function TRegex.Replace(const Input, Replacement: String; const Count,
  StartAt: Integer): String;
var
  MatchCount: Integer;
begin
  if (Count = 0) then
  ...
  SetSubject(Input);
  FStartOffset := StartAt;
  FReplacement := UTF8Encode(Replacement);
  ...
  Result := UTF8ToString(FSubject);
end;

function TRegex.Replace(const Input: String; const Evaluator: TMatchEvaluator;
  const Count, StartAt: Integer): String;
begin
  FEvaluator := Evaluator;
  try
...
procedure TRegex.SetSubject(const Value: String);
begin
  FSubject := UTF8Encode(Value);
  FStartOffset := 1;
  FMaxLength := Length(FSubject);
  FOffsetCount := 0;
end;

class function TRegex.Split(const Input, Pattern: String;
...
function TRegex.Split(const Input: String; const Count,
...
begin
  Result := UTF8ToString(FPattern);
end;

class function TRegex.Unescape(const Str: String): String;
var
  I, J, K, C, D, E: Integer;
begin
  Result := Str;
...
constructor TRegex.TCachedCompiledPattern.Create(const AKey: String;
...
procedure TRegex.TAnsiStringBuilder.Append(const Value: AnsiString;
...
function TRegex.TAnsiStringBuilder.GetValue: AnsiString;
begin
  if (FBuffer = nil) then
    Result := ''
  else
...
constructor TCapture.Create(const AValue: AnsiString; const AIndex,
  ALength: Integer);
begin
  inherited Create;
  FValue := AValue;
...
function TCapture.GetValue: String;
begin
  if (FIndex <= 0) then
    Result := ''
  else
    Result := UTF8ToString(Copy(FValue, FIndex, FLength));
end;
...
function TGroupCollection.GetItemByName(const GroupName: String): IGroup;
var
...
//  GroupNum := FMatch.FRegex.GroupNumberFromName(GroupName);
//  if (GroupNum < 0) or (GroupNum >= FCount) then
//    Result := TGroup.EmptyGroup
//  else
//    Result := FItems[GroupNum];
end;
...
constructor TMatch.Create(const ARegex: TRegex; const AValue: AnsiString;
  const AIndex, ALength: Integer);
begin
...
function TMatch.Result(const Replacement: String): String;
begin
  if (FRegex = nil) then
    raise EArgumentException.Create(RS_ARGUMENT_NIL);
  Result := UTF8ToString(FRegex.ExpandSubstitutions(UTF8Encode(Replacement)));
end;
