...
uses
  WinTypes, SysUtils, Messages, Classes, Vcl.Graphics, Vcl.Controls,
  Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls, Vcl.Buttons, ColorUtils;
...
{$IFDEF DFS_COMPILER_4_UP}
  if BiDiMode = bdRightToLeft then
    solvedRect := Rect(barRect.right - Trunc((barRect.right - barRect.left) / (FMaxValue - FMinValue) * FProgress), barRect.top, barRect.right, barRect.bottom)
  else
    solvedRect := Rect(barRect.left, barRect.top, barRect.left + Trunc((barRect.right - barRect.left) / (FMaxValue - FMinValue) * FProgress), barRect.bottom);
{$ELSE}
  solvedRect := Rect(barRect.left, barRect.top, barRect.left + Trunc((barRect.right - barRect.left) / (FMaxValue - FMinValue) * FProgress), barRect.bottom);
{$ENDIF}
  // Fill the Rect
...
procedure Register;
begin
  RegisterComponents('cactus', [TFlatGauge]);
end;
