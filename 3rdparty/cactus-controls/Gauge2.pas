...
uses SysUtils, Windows, Messages, Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms,
  Vcl.Dialogs,  Vcl.ExtCtrls, Vcl.Buttons, ColorUtils;
...
type
  ...
  private
    ...
    FForeColor: TColor;
    FBackColor: TColor;
    procedure PaintBackground(MemBitmap: TBitmap);
    procedure PaintAsText(MemBitmap: TBitmap; PaintRect: TRect);
    procedure PaintAsNothing(MemBitmap: TBitmap; PaintRect: TRect);
    procedure PaintAsBar(MemBitmap: TBitmap; PaintRect: TRect);
    procedure PaintAsPie(MemBitmap: TBitmap; PaintRect: TRect);
    procedure PaintAsNeedle(MemBitmap: TBitmap; PaintRect: TRect);
  ...
  begin
    ...
    try
      ...
      try
        ...
        memBitmap.Canvas.CopyMode := cmSrcInvert;
        memBitmap.Canvas.Draw(0, 0, OverlayImage);
        memBitmap.Canvas.CopyMode := cmSrcCopy;
        if ShowText then PaintAsText(memBitmap, PaintRect);
      finally
        OverlayImage.Free;
      end;
...
procedure TGauge2.PaintBackground(MemBitmap: TBitmap);
var
  ARect: TRect;
begin
  ...
  begin
    ...
    CopyRect(ARect, MemBitmap.Canvas, ARect);
    CopyMode := cmSrcCopy;
  end;
end;

procedure TGauge2.PaintAsText(MemBitmap: TBitmap; PaintRect: TRect);
var
  S: string;
  X, Y: Integer;
  OverRect: TBltBitmap;
begin
  ...
  try
    ...
    MemBitmap.Canvas.CopyMode := cmSrcInvert;
    MemBitmap.Canvas.Draw(0, 0, OverRect);
  finally
    OverRect.Free;
  end;
end;

procedure TGauge2.PaintAsNothing(MemBitmap: TBitmap; PaintRect: TRect);
begin
  with MemBitmap do
  begin
...
procedure TGauge2.PaintAsBar(MemBitmap: TBitmap; PaintRect: TRect);
var
  FillSize: Longint;
  W, H: Integer;
begin
...
procedure TGauge2.PaintAsPie(MemBitmap: TBitmap; PaintRect: TRect);
var
...
procedure TGauge2.PaintAsNeedle(MemBitmap: TBitmap; PaintRect: TRect);
var
  MiddleX: Integer;
  Angle: Double;
  X, Y, W, H: Integer;
...
procedure Register;
begin
  RegisterComponents('cactus', [TGauge2]);
end;
