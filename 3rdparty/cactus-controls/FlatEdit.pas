...
uses
  Windows, Messages, Classes, Vcl.Graphics, Vcl.Controls, Vcl.Forms,
  Vcl.Dialogs,  Vcl.ExtCtrls, Vcl.StdCtrls, ColorUtils;
...
begin
  ...
  try
    ...
    if (not (csDesigning in ComponentState) and (Focused or (MouseInControl and not (Screen.ActiveControl is TFlatEdit)))) then
    begin
      { Focus }
      Color := FFocusedColor;
...
procedure Register;
begin
  RegisterComponents('cactus', [TFlatEdit]);
end;
